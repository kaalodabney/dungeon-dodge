...
def playerCollision args, p, e, dir
     ...
     if p.colRect.intersect_rect? e
      args.outputs.labels << [10, 540, "Player colliding #{dir}"]
      return dir
     else
      return nil
     end
...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    if args.inputs.up 
      args.state.enemies.each { |e| p.col = playerCollision(args, p, e, "up")}
      args.outputs.labels << [10, 500, "Player.col: #{args.state.players[0].col} #{p.col}"]
    ...
    if args.inputs.right 
      args.state.enemies.each { |e| p.col = playerCollision(args, p, e, "right")}
    ...
    if args.inputs.down 
      args.state.enemies.each { |e| p.col = playerCollision(args, p, e, "down")}
    ...
    if args.inputs.left 
      args.state.enemies.each { |e| p.col = playerCollision(args, p, e, "left")}
