...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    args.state.keypressed.include?("up")    ? p.y += p.dy : nil
    args.state.keypressed.include?("down")  ? p.y -= p.dy : nil
    args.state.keypressed.include?("right") ? p.x += p.dx : nil
    args.state.keypressed.include?("left")  ? p.x -= p.dx : nil
