...
def playerCollision args, p, e
  ...
  if args.state.keypressed.include?("left")
    p.debugTog1 ? (p.debugTog1 = true) : (p.debugVal1 = [p.colRect.x, p.x]; p.debugTog1 = true)
    if p.debugTog4
      p.debugVal4 += 1
    end
    p.colRect.x = (p.x - p.dx)
...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    args.outputs.labels << [10, 100, "colRect.x, p.x: keypressed array includes left (in playerCollision): #{p.debugVal1} (x should be 590)"]
