...
class Enemy < Entity
    ...
    def enemyCollision e
        collision = false
        if @colArray.include?("right") || @colArray.include?("left")
            @colRect.x = @x + @dx 
            collision = @colRect.intersect_rect? e.colRect
            @args.outputs.labels << [10, 460, "e.collision: #{collision}"]
        end
        
        if @colArray.include?("up") || @colArray.include?("down")
            @colRect.y = @y + @dy 
            collision = @colRect.intersect_rect? e.colRect
        end

        collision ? (@colArray.push(e) unless @colArray.include?(e)) : (@colArray.delete(e) if @colArray.include?(e))
        @args.outputs.labels << [10, 480, "@colArray: #{@colArray}"]
    ...
    def movement p, enemyArray
        ...
        colBuffer = false # A buffer to detect if collision is true for ANY enemies, not just the last one, in a certain direction
        enemyArray.each do |e|
            enemyCollision(e) unless e.id == @id
            colBuffer = true if @col
        end
        @col = colBuffer
        @args.outputs.labels << [10, 500, "e.@col: #{@col}"]
        ...
        # @colDir.each do |dir|
        #     @dx = 0 if @col && ["right", "left"].include?(dir)
        #     @dy = 0 if @col && ["up", "down"].include?(dir)
        # end
