...
class Game
  ...
  def renderDebugInfo
    ...
    debug.labels << {x: 10, y: 540, text: "cameraFocus: [x: #{@cameraFocusX}, y: #{@cameraFocusX}]", r: 255, g: 255, b: 255}    
  end

  def cameraFocus
    @cameraFocusX = (1280 * 2 * @zoom) / (@player.x)
    @cameraFocusY = (720 * 2 * @zoom) / (@player.y)
  end
  ...
  def renderMap
    ...
    outputs.sprites << [@cameraFocusX *@zoom, @cameraFocusY*@zoom, mapW, mapH, :map]
  end
