...
def playerCollision args, p, e, dir
     ...
     (p.colRect.intersect_rect? e) ? (args.outputs.labels << [10, 520, "Collision #{dir}"]; true) : false
...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    if args.inputs.up 
      args.state.enemies.each { |e| p.col = "up" if playerCollision(args, p, e, "up") }
      args.outputs.labels << [10, 500, "#{p.col}"]
      if !args.state.keypressed.include?("up") 
        args.state.keypressed.push("up")
        p.dy += 5 if p.col != "up" #move up 5 if no collision in up direction
      end
      upPressed = true
    end 
    ...
    if args.inputs.right 
      args.state.enemies.each { |e| p.col = "right" if playerCollision(args, p, e, "right")}
      args.outputs.labels << [10, 500, "#{p.col}"]
      if !args.state.keypressed.include?("right")
        args.state.keypressed.push("right")

        p.dx += 5 if p.col != "right"
      end
      p.spriteFlip = false
      rightPressed = true
    end
    ...
    if args.inputs.down 
      args.state.enemies.each { |e| p.col = "down" if playerCollision(args, p, e, "down")}
      args.outputs.labels << [10, 500, "#{p.col}"]
      if !args.state.keypressed.include?("down") 
        args.state.keypressed.push("down") 
        p.dy -= 5 if p.col != "down"
      end
      downPressed = true
    end     
    ...
    if args.inputs.left 
      args.state.enemies.each { |e| p.col = "left" if playerCollision(args, p, e, "left")}
      args.outputs.labels << [10, 500, "#{p.col}"]
      if !args.state.keypressed.include?("left")
        args.state.keypressed.push("left")
        p.dx -= 5 if p.col != "left"
      end
      p.spriteFlip = true
      leftPressed = true
    end
