...
def playerCollision args, p, e
  ...
  args.state.keypressed.reverse().each do |k|
    (p.colRect.y = p.y + p.dy; up = true) if k == "up"
    (p.colRect.x = p.x + p.dx; right = true) if k == "right"
    (p.colRect.y = p.y + p.dy; down = true)if k == "down" 
    (p.colRect.x = (p.x + p.dx); left = true)if k == "left"
  end
