...
class Game
  ...
  def blowUpBomb bomb
      expl = Effect.new(id: @entityCount, type: "explosion", x: bomb.x, y: bomb.y)
  ...
  def entityUpdate
    ...
    @entityArray.each do |e|
      ...
      if e.is_a? Player
        ...
        if e.dead 
          @entityArray.delete(e)
          next
        end
        e.movement @entityArray 
      end
    end
    ...
    @entityArray.each do |e|
      ...
      if (state.tick_count % frameSpeed) == 0
        ...
        if e.spriteID >= frameCount
          ...
          @entityArray.delete(e) if e.type == "explosion"
  ...
  def diagnosticsLabels
    ...
    outputs.labels << {x:640, y: 60, text: "Press [c] to #{@keypressed.include?("c") ? "disable" : "enable"} player collision.", alignment_enum: 1}
    outputs.labels << {x:640, y: 30, text: "Press [space] = jump | [esq] = reset", alignment_enum: 1}    
  end
