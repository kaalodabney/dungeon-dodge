...
class Game
  attr_gtk
  attr_accessor :player, :enemyArray, :keypressed, :bombArray, :entityCount, :map, :outputs, :cameraFocusX, :cameraFocusY, :cameraZoom, :debug
  def defaults
    puts "defaults"
    @cameraZoom = 2
    @keypressed ||= Array.new(0)
    @entityCount ||= 0
    @player ||= Player.new(id: @entityCount, type: "player", x: 1185, y: 545, movespeed: 3, zoom: @cameraZoom)
    @entityCount += 1
    @entityArray  ||= [ @player,
                    Enemy.new(id: @entityCount, type: "bat", x: 100, y: 600, movespeed: 2, zoom: @cameraZoom),
                    Enemy.new(id: @entityCount+1, type: "slime", x: 1100, y: 600, zoom: @cameraZoom),
                    Enemy.new(id: @entityCount+2, type: "goblin", x: 300, y: 600, movespeed: 2, zoom: @cameraZoom),
                    Enemy.new(id: @entityCount+3, type: "bat", x: 400, y: 550, movespeed: 2, zoom: @cameraZoom),
                    Enemy.new(id: @entityCount+4, type: "slime", x: 500, y: 600, zoom: @cameraZoom),
                    Enemy.new(id: @entityCount+5, type: "bat", x: 900, y: 400, zoom: @cameraZoom),
                    Enemy.new(id: @entityCount+6, type: "goblin", x: 200, y: 100, zoom: @cameraZoom),
                    Enemy.new(id: @entityCount+7, type: "bat", x: 200, y: 100, zoom: @cameraZoom),
                    Enemy.new(id: @entityCount+8, type: "slime", x: 200, y: 90, zoom: @cameraZoom),
                    Enemy.new(id: @entityCount+9, type: "bat", x: 201, y: 100, zoom: @cameraZoom)                  
                  ]
    @entityCount += 10              
    @upPressed ||= false
    @downPressed ||= false
    ...
    @map.load
    @cameraFocusX ||= (-1 * player.x.to_f) + (1280 / @cameraZoom)
    @cameraFocusY ||= (-1 * player.y.to_f) + (720 / @cameraZoom)
    puts "eeeeeeeeeeeeeeeeeeeeeeeeeeee #{@entityArray}"
  end
  ...
  def renderMap
    outputs.sprites << [0, 0, 1280, 720, :map]
    if state.tick_count.zero?
      attributes = @map.attributes
      outputs[:map].w = 1280 / cameraZoom
      outputs[:map].h = 720 / cameraZoom
      outputs[:map].sprites << map.layers.map(&:sprites)
    end
  end

  def cameraFocus
    @cameraFocusX = (-1 * @player.x) + 640
    @cameraFocusY = (-1 * @player.y) + 360
  end
