...
class Game
  ...
  def defaults
    ...
    @entityCount ||= 0
    @player ||= Player.new(id: @entityCount, type: "player", x: 1280/2, y: 720/2, movespeed: 3,)
    @entityCount += 1
    @entityArray  ||= [ @player,
  ...
  def cameraFocus
    @cameraFocusX = -(@player.x)*@zoom
    @cameraFocusY = -(@player.y)*@zoom
  end

  def renderMap
    if state.tick_count.zero?
      outputs[:map].w = 1280/@zoom*2
      outputs[:map].h = 720/@zoom*2
    ...
    outputs.sprites << [@cameraFocusX-1025, @cameraFocusY-325, mapW, mapH, :map]
  end
