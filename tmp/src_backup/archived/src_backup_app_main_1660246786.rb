...
class Game
  ...
  def tick
    ...
    if state.tick_count.zero?
      ...
      outputs[:map].w = @map.attributes.width.to_i * attributes.tilewidth.to_i
      outputs[:map].h = @map.attributes.height.to_i * attributes.tileheight.to_i
      outputs[:map].x = @map.attributes.height.to_i * attributes.tileheight.to_i
      outputs[:map].y = @map.attributes.height.to_i * attributes.tileheight.to_i
      

      outputs[:map].sprites << map.layers.map(&:sprites)
      outputs.static_sprites << [(1280 - 720)/2, 0, 720, 720, :map]
