...
class Game
  ...
  def blowUpBomb bomb
      @entityCount -= 1
      @effectsArray.push(Effect.new(id: @entityCount, type: "explosion", x: bomb.x, y: bomb.y))
      @entityCount += 1
  ...
  def entityUpdate
    ...
    @entityArray.each do |e|
      ...
      puts "player: #{e.is_a? Player}" #e.playerMovement @keypressed, @entityArray 
      puts "enemy: #{e.is_a? Enemy}" #e.movement @player, @entityArray 
    end
    ...
    @entityArray.each do |e|
      ...
      if (state.tick_count % frameSpeed) == 0
        ...
        if e.spriteID >= frameCount
          ...
          @effectsArray.delete(e) if e.type == "explosion"
