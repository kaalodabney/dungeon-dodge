...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    if args.inputs.right 
      ...
      if not args.state.keypressed.include?("right")
        args.state.keypressed.push("right")
        p.dx += 5 unless p.col[0]
      end
      p.spriteFlip = false
      rightPressed = true
    end
    ...
    if args.inputs.left 
      p.col = playerCollisionRect(args, p, "left")

      if not args.state.keypressed.include?("left")
    ...
    if args.inputs.up 
      p.col = playerCollisionRect(args, p, "up")
      if not args.state.keypressed.include?("up") 
        args.state.keypressed.push("up")
        p.dy += 5 unless p.col[0]
      end
      upPressed = true
    end 
    # down movement and animation state change
    if args.inputs.down 
      p.col = playerCollisionRect(args, p, "down")
      if not args.state.keypressed.include?("down") 
