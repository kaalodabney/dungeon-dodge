...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    args.state.keypressed.delete("up") if !upPressed
    args.state.keypressed.delete("down") if !downPressed
    args.state.keypressed.delete("right") if !rightPressed
    args.state.keypressed.delete("left") if !leftPressed

    # if player not pressing (up/down) or (left/right), set (y/x) speed to 0
    (args.state.keypressed.include?("up") && (p.colDir != "up")) || (args.state.keypressed.include?("down") && (p.colDir != "down")) ? p.dy = moveSpeed : p.dy = 0
    (args.state.keypressed.include?("right") && (p.colDir != "right")) || (args.state.keypressed.include?("left") && (p.colDir != "up")) ? p.dx = moveSpeed : p.dx = 0 
...
def diagnosticsLabels args
    ...
    args.outputs.labels << [10, 540, "Player.colDir: #{args.state.players[0].colDir}"]
end
