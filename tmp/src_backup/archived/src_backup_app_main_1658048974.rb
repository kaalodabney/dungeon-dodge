...
def playerCollision args, p, dir
    ...
    dir == "up" ? pClone.y = p.y+p.speed : nil
    dir == "down" ? pClone.y = p.y-p.speed : nil
    dir == "right" ? pClone.x = p.x+p.speed : nil
    dir == "left" ? pClone.x = p.x-p.speed : nil

    args.state.enemies.each do |e|

      (e.intersect_rect? pClone) ? (p.collision = true; e.collision = true) : p.collision = false; e.collision = false
...
def playerInput args
  ...
  args.state.players.each do |p|
    ...
    if args.inputs.up # up movement and animation state change
      ...
      playerCollision(args, p, "up")
      p.collision ? p.speed = 0 : nil
      p.y += p.speed
      upDownPressed = true
    end 
    ...
    upDownPressed || rightLeftPressed ? nil : (p.spriteAnim = "idle"; args.state.keypressed = ""; p.speed = 0)
  end
end
...
def diagnosticsLabels args
    ...
    args.state.players[0].collision ? (args.outputs.labels << [10, 580, "Collision"]) : nil
end
