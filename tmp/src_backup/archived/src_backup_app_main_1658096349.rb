...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    if args.inputs.up 
      p.dy = moveSpeed
      args.state.keypressed.push("up") unless p.col && args.state.keypressed.include?("up") 
    ...
    if args.inputs.down 
      p.dy = moveSpeed
      args.state.keypressed.push("down") unless p.col && args.state.keypressed.include?("down") 
      playerCollisionRect(args, p, "down")

      downPressed = true
    end  

    p.x += p.dx unless p.col && args.state.keypressed.include?("right") 
    p.x -= p.dx unless p.col && args.state.keypressed.include?("left") 
    p.y += p.dy unless p.col && args.state.keypressed.include?("up") 
    p.y -= p.dy unless p.col && args.state.keypressed.include?("down") 
    ...
    p.dy = 0 unless args.state.keypressed.include?("up") || args.state.keypressed.include?("down")
    p.dx = 0 unless args.state.keypressed.include?("right") || args.state.keypressed.include?("left")
