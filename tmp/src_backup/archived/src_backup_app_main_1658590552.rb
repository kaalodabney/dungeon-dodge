...
def spriteAnimations args
  x=5 #6 frame animation speed (player, goblin, slime)
  y=3 #4 frame animation speed (bat)

  # increment each player sprite every x ticks
  args.state.players.each do |p|
  ...
  args.state.enemies.each do |e|
    ...
    args.outputs.labels << [900, 520 - (e.id * 20), "[sID:#{e.spriteID}, id:#{e.id}, et: #{e.enemyType}], "] 
    if e.enemyType == "goblin" || e.enemyType == "slime"
      if args.state.tick_count % x == 0
        e.spriteID += 1
        if e.spriteID > 5
          e.spriteID = 0
    ...
    if e.enemyType == "bat"
      if args.state.tick_count % y == 0
         e.spriteID += 1
        if e.spriteID > 3
            e.spriteID = 0
        end

        if args.state.debug.tog == false
          args.state.debug.val = "b: #{e.spriteID}"
          args.state.debug.tog = true
        end

        args.outputs.labels << [10, 540, "#{args.state.debug.val}, id:#{e.id}"] 
      end
    end
 ...
 #loop through npcs, set sprite path depending on current spriteAnim and enemyType
  args.state.enemies.each do |e|
    ...
    if e.spriteAnim == "idle"
      e.sprite = "sprites/dc16pp/enemies/goblin/goblin_idle_anim_f#{e.spriteID}.png" if e.enemyType == "goblin"
      e.sprite = "sprites/dc16pp/enemies/slime/slime_idle_anim_f#{e.spriteID}.png" if e.enemyType == "slime"
    end

    if e.spriteAnim == "walk"
      e.sprite = "sprites/dc16pp/enemies/goblin/goblin_run_anim_f#{e.spriteID}.png" if e.enemyType == "goblin"
      e.sprite = "sprites/dc16pp/enemies/slime/slime_run_anim_f#{e.spriteID}.png" if e.enemyType == "slime"
    end

   e.sprite = "sprites/dc16pp/enemies/flying creature/fly_anim_f#{e.spriteID}.png" if e.enemyType == "bat"
  end
