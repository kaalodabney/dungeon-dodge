...
def playerCollision args, dir
  args.state.players.each do |p|
    ...
    dir == "up" ? pClone = [p.x, p.y+p.speed, p.w, p.h] : nil
    dir == "down" ? pClone = [p.x, p.y-p.speed, p.w, p.h] : nil
    dir == "right" ? pClone = [p.x+p.speed, p.y, p.w, p.h] : nil
    dir == "up" ? pClone = [p.x-p.speed, p.y, p.w, p.h] : nil
...
def playerInput args
  ...
  args.state.players.each do |p|
    ...
    upDownPressed || rightLeftPressed ? nil : (p.spriteAnim = "idle"; args.state.keypressed = "", p.)
  end
end
