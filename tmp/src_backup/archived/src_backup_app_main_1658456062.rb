...
def entityController args
  ...
  args.state.players.each do |p|
    args.outputs.labels << [10, 80, "[colRect.x, p.x]: First Left input (before collision check): #{p.debugVal3}"]
    args.outputs.labels << [10, 60, "[colRect.x, p.x]: keypressed array includes left (in playerCollision): #{p.debugVal1} (colRect.x should be 585, p.xx should be 590)"]
    args.outputs.labels << [10, 40, "left input count (in playerCollision): #{p.debugVal4}"]
