...
def playerCollision args, p, e
  ...
  if args.state.keypressed.include?("left")
    p.debugTog ? (p.debugTog = true) : (p.debugVal = "colRect: #{p.colRect.dup}, p.x p.y: #{p.x.dup} #{p.y.dup}"; p.debugTog = true)
    p.colRect.x = p.x - p.dx
    p.debugTog2 ? (p.debugTog2 = true) : (p.debugVal2 = "colRect: #{p.colRect.dup}, p.x p.y: #{p.x.dup} #{p.y.dup}"; p.debugTog2 = true)
    left = true
  end
