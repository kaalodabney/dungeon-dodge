...
def determineCollision args
  ...
  args.state.players.each do |p|
    (args.state.enemies.any_intersect_rect? p) ? p.collision = true : p.collision = false
  end
...
def playerInput args
  ...
  args.state.players.each do |p|
    if args.inputs.up # up movement and animation state change
      ...
      p.y += p.speed
      p.collision ? (p.y -= p.speed; p.speed = 0; p.collision = false) : p.speed = 5 
      upDownPressed = true
    elsif args.inputs.down #down movement and animation state change
      p.spriteAnim = "walk"; args.state.keypressed = "down"
      p.y -= p.speed
      p.collision ? (p.y += p.speed*2; p.speed = 0; p.collision = false) : p.speed = 5 
      upDownPressed = true
    else
      upDownPressed = false
    end
    ...
    if args.inputs.right # right movement and animation state change
      ...
      p.x += p.speed
      p.collision ? (p.x -= p.speed; p.speed = 0; p.collision = false) : p.speed = 5 
      p.spriteFlip = false
