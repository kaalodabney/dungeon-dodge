...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    args.state.keypressed.each do |k|
      (p.dy = 5 if p.col) if  k == "up"    #p.colDir.include?("up")) if  k == "up"    
      (p.dx = 5 if p.col; p.spriteFlip = false) if  k == "right" #p.colDir.include?("right"); p.spriteFlip = false) if  k == "right"
      (p.dy = -5 if p.col) if k == "down"
      (p.dx = -5 if p.col; p.spriteFlip = true) if k == "left"
    end
