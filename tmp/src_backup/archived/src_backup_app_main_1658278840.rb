...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    args.state.keypressed.each do |k|
      k == "up"    ? (p.dy = 5 unless p.colDir.include?("up")) : (args.state.keypressed.delete("up") if args.state.keypressed.include?("up"))
      k == "right" ? (p.dx = 5 unless p.colDir.include?("right")) : (args.state.keypressed.delete("right") if args.state.keypressed.include?("right"))
      k == "down"  ? (p.dy = -5 unless p.colDir.include?("down")) : (args.state.keypressed.delete("down") if args.state.keypressed.include?("down"))
      k == "left"  ? (p.dx = -5 unless p.colDir.include?("left")) : (args.state.keypressed.delete("left") if args.state.keypressed.include?("left"))
    end
