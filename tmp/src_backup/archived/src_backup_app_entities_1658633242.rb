...
class Enemy < Entity
    ...
    def enemyCollision e
        ...
        @colRect.x += @dx
        xCollision = [@colRect.x-1, @colRect.y-1, @colRect.w+2, @colRect.h+2].intersect_rect? e.colRect
        @colRect.x = @x

        @colRect.y += @dy
        yCollision = [@colRect.x-1, @colRect.y-1, @colRect.w+2, @colRect.h+2].intersect_rect? e.colRect
        @colRect.y = @y

        @colRect.x += @dx
        @colRect.y += @dy
        xyCollision = [@colRect.x-1, @colRect.y-1, @colRect.w+2, @colRect.h+2].intersect_rect? e.colRect
        @colRect.y = @y
        @colRect.x = @x
        ...
        @dx = 0 if xCollision || xyCollision
        @dy = 0 if yCollision || xyCollision

    end

    def enemyMovement target, entitiesArray
        movespeed = 2
       
     
        @dx = movespeed if @x < target.x
        @dx = -movespeed if @x > target.x
        @dy = movespeed if @y < target.y
        @dy = -movespeed if @y > target.y
