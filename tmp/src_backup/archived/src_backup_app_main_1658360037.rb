...
def playerCollision args, p, e
  ...
  args.outputs.labels << [10, 400, "#{p.colRect}"]
  args.outputs.labels << [10, 420, "#{p.x}, #{p.y}"]
...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    p.colRect = {x: p.x, y: p.y, w: p.h, h: p.h}
    p.colDir.clear
    p.col = false
    p.dx = 0
    p.dy = 0
    ...
    args.outputs.labels << [10, 380, "#{p.colRect}"]
    args.outputs.labels << [10, 360, "#{p.x}, #{p.y}"]
