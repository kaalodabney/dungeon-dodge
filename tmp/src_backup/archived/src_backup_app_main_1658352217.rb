...
def playerCollision args, p, e
  ...
  p.colRect = {x: p.x.dup, y: p.y.dup, w: p.w.dup, h: p.h.dup }
...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    args.state.keypressed.each do |k|
      (p.dy = 5 unless p.colDir.include?("up")) if  k == "up"    
      (p.dx = 5 unless p.colDir.include?("right"); p.spriteFlip = false) if  k == "right"
      (p.dy = -5 unless p.colDir.include?("down")) if k == "down"
      (p.dx = -5 unless p.colDir.include?("left"); p.spriteFlip = true) if k == "left"
    end
