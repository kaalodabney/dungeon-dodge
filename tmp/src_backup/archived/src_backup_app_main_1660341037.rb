...
class Game
  attr_gtk
  attr_accessor :outputs, :player, :enemyArray, :keypressed, :bombArray, :entityCount, :map, :cameraFocusX, :cameraFocusY, :debug
  def defaults
    @zoom ||= 2
    @tileSize ||= 16
    @keypressed ||= Array.new(0)
    @entityCount ||= 0
    @player ||= Player.new(id: @entityCount, type: "player", x: 1280- 64, y: 720-64, movespeed: 3,)
    @entityCount += 1
    @entityArray  ||= [ @player,
    ...
    @entityArray.each do |e|
      e.w = @tileSize.to_i * @zoom.to_i * 2
      e.h = @tileSize.to_i * @zoom.to_i * 2
    end
  end
  ...
  def renderDebugInfo
    ...
    debug.labels << {x: 10, y: 540, text: "cameraFocus: [x: #{@cameraFocusX}, y: #{@cameraFocusY}]", r: 255, g: 255, b: 255}    
  end

  def cameraFocus
    @cameraFocusX = 1280-(@player.x)/(2/@zoom)
    @cameraFocusY = 720-(@player.y)/(2/@zoom)
  end

  def renderMap
    if state.tick_count.zero?
      outputs[:map].w = 1280/(4/@zoom)
      outputs[:map].h = 720/(4/@zoom)
      outputs[:map].sprites << map.layers.map(&:sprites)
    end
    mapW = (@map.width.to_i * 2 * @map.tilewidth.to_i * @zoom.to_i)
    mapH = (@map.height.to_i * 2 * @map.tileheight.to_i * @zoom.to_i)
    outputs.sprites << [@cameraFocusX, @cameraFocusY, mapW, mapH, :map]
  end
...
def tick args
  ...
  args.outputs.sprites << [0, 0, 1280, 720, :camera]
