...
def spriteAnimations args
  x.speed = 5 # (player, goblin, slime) sprite animation speed
  x.count = 5 # (player, goblin, slime) sprite animation frame count
  y.speed = 4 # (bat) sprite animation speed
  y.count = 3 # (bat) sprite animation frame count

  # increment each player sprite every x ticks
  args.state.players.each do |p|
    if args.state.tick_count % x.speed == 0
      p.spriteID += 1
      if p.spriteID > x.count
        p.spriteID = 0
      end
    end
  end
