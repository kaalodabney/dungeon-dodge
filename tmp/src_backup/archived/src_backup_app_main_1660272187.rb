...
class Game
  ...
  def defaults
    ...
    @entityCount ||= 0
    @player ||= Player.new(id: @entityCount, type: "player", x: 590, y: 250, movespeed: 3)
    @entityCount += 1
    @entityArray  ||= [ @player,
  ...
  def renderDebugInfo
    #Stats Labels
    debug.labels << {x: 10, y: 720, text: "Tick Counter: #{state.tick_count}"}
    debug.labels << {x: 10, y: 700, text: "FPS: #{gtk.current_framerate.round}"}
    debug.labels << {x: 10, y: 680, text: "SpriteID: #{@player.spriteID}"}
    debug.labels << {x:10, y: 660, text: "Sprite: #{@player.sprite}"}
    debug.labels << {x:10, y: 640, text: "Key Pressed: #{@keypressed}"}
    debug.labels << {x:10, y: 620, text: "Animation: #{@player.spriteAnim}"}
    debug.labels << {x:10, y: 600, text: "Player.dx: #{@player.dx}"}
    debug.labels << {x:10, y: 580, text: "Player.dy: #{@player.dy}"}
    debug.labels << {x:640, y: 60, text: "[c] to #{@keypressed.include?("c") ? "disable" : "enable"} player collision.", alignment_enum: 1}
    debug.labels << {x:640, y: 30, text: "[space/shift] = bomb | [esc] = reset", alignment_enum: 1}    
  end

  def renderMap
    if state.tick_count.zero?
      attributes = @map.attributes
      outputs[:map].w = 1280/2
      outputs[:map].h = 720/2
  ...
  def cameraFocus
    @cameraFocusX = @player.x - 1280
    @cameraFocusY = @player.y - 720
  end
...
def tick args
  ...
  $game.args = args
  $game.tick
  args.outputs.sprites << [0, 0, :debugInfo]
  args.outputs.sprites << [$game.cameraFocusX, $game.cameraFocusY, 2560, 1420, :camera]
