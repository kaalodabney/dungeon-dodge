...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    args.state.keypressed.include?("up") || args.state.keypressed.include?("down") ? p.dy = moveSpeed : p.dy = 0
    args.state.keypressed.include?("right") || args.state.keypressed.include?("left") ? p.dx = moveSpeed : p.dx = 0 

    args.state.keypressed.include?("up")    && !p.col && p.colDir != "up" ? p.y += p.dy : nil
    args.state.keypressed.include?("down")  && !p.col && p.colDir != "down"? p.y -= p.dy : nil
    args.state.keypressed.include?("right") && !p.col && p.colDir != "right" ? p.x += p.dx : nil
    args.state.keypressed.include?("left")  && !p.col && p.colDir != "left"? p.x -= p.dx : nil
