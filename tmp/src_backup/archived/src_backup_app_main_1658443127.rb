...
def actorController args
  ...
  args.state.players.each do |p|
    args.outputs.labels << [10, 440, "debugVal - keypressed includes left (in playerCollision): #{p.debugVal}"]
    args.outputs.labels << [10, 420, "debugVal2 - : #{p.debugVal2}"]
    args.outputs.labels << [10, 400, "debugVal3 - left input (before collision check): #{p.debugVal3}"]
    args.outputs.labels << [10, 380, "debugVal4 - left input count: #{p.debugVal4}"]
