...
def playerCollisionRect args, p, dir
  pClone = {x: p.x, y: p.y, w: p.w, h: p.h}
    ...
    args.state.enemies.each do |e|
      ...
      if dir == "right"
        pClone.x = p.x+p.dx
        (pClone.intersect_rect? e) ?  p.collisionDirection = "right" : nil
      end
        
      if dir == "left"
        pClone.x = p.x-p.dx
        (pClone.intersect_rect? e) ? p.collisionDirection = "left" : nil
      end

      if dir == "up"
        pClone.y = p.y+p.dy
        (pClone.intersect_rect? e) ? p.collisionDirection = "up" : nil
      end

      if dir == "down"
        pClone.y = p.y-p.dy
        (pClone.intersect_rect? e) ? p.collisionDirection = "down" : nil
      end

      if p.collisionDirection != "" 
        e.collision = true
        p.collision = true
      end
