...
class Game
  ...
  def defaults
    ...
    @entityCount ||= 0
    @player ||= Player.new(id: self.entityCount, type: "player", x: 590, y: 250, w:75, h:75, movespeed: 6)@entityCount += 1
  ...
  def blowUpBomb bomb
      @entityCount -= 1
      @effectsArray.push(Effect.new(id: @entityCount, x: bomb.x, y: bomb.y))
      @entityCount += 1
      @bombArray.delete(bomb)
