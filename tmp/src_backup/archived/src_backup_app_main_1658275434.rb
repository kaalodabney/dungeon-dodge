...
def playerCollision args, p, e
  ...
  args.state.keypressed.include?("right") ? (p.colRect.x = p.x + p.dx; p.colDir.push("right")) : nil
  args.state.keypressed.include?("left") ? (p.colRect.x = p.x - p.dx; p.colDir.push("left")) : nil
  args.state.keypressed.include?("up") ? (p.colRect.y = p.y + p.dy; p.colDir.push("up")) : nil
  args.state.keypressed.include?("down") ? (p.colRect.y = p.y - p.dy; p.colDir.push("right")) : nil

  (p.colRect.intersect_rect? e) ? true : false
...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    p.dy += 5 unless p.colDir.includes?("up") #move up 5 if no collision in up direction
    p.dx += 5 unless p.colDir.includes?("right")
    p.dy -= 5 unless p.colDir.includes?("down")
    p.dx -= 5 unless p.colDir.includes?("left")
    ...
    if ["right","left"].include?(p.col)
      args.outputs.labels << [10, 500, "right/left: #{p.col == nil ? "nil" : p.col}"]
      p.dx = 0
    end

    if ["up","down"].include?(p.col)
      p.dy = 0
    end
