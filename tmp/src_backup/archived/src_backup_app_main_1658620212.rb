...
class Game
     ...
     #Loop through input stack
      ...
      @keypressed.each do |k|
        ...
        @enemyArray.each do |e| 
          ...
          (p.colArray.push(e) unless p.colArray.include?(e)) if e.col 
  ...
  def playerDamage p
    ...
    if p.healthbar.w > 20 && p.colArray.length() > 0
      p.healthbar.w -=  (0.5 * (p.colArray.length() - (0.2 * p.colArray.length())))
    elsif p.colArray.length() > 0
  ...
  def entityUpdate args
    ...
    playerInput args #detects player input and pushes to input stack
