...
def playerCollision args, p, e
  ...
  args.state.keypressed.include?("down") ? (p.colRect.y = p.y - p.dy; p.colDir.push("down")) : nil

  (p.colRect.intersect_rect? e) ? true : (p.colDir.clear(); false)
...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    p.dy += 5 unless p.colDir.include?("up") #move up 5 if no collision in up direction
    ...
    args.outputs.labels << [10, 480, "p.colDir: #{p.colDir}"]
    if (["right","left"] & p.colDir).any? #tests if right/left is in the p.colDir array
      args.outputs.labels << [10, 500, "right/left: #{p.col == nil ? "nil" : p.col}"]
      p.dx = 0
    end

    if (["up","down"] & p.colDir).any?
      p.dy = 0
    end
