class Entity
    ...
    def initialize id:, type:, x:, y:, w: 16, h: 16, movespeed:0, spriteAnim: "idle", spriteDir: "right"
        @id = id
        @type = type
        @x =  x
        @y =  y
...
class Player < Entity
    ...
    def initialize id:, type: "player", x:, y:, w: , h: , movespeed:5 
...
class Enemy < Entity
    attr_accessor :dead
    def initialize id: , type: , x: , y: , w: , h: , movespeed: 1
        super(id: id, type: type, x: x, y: y, w: w, h: h, movespeed: movespeed)
        @hp ||= 25
        @dead ||= false
    end
