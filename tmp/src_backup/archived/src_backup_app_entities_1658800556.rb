...
class Entity
    ...
    def collision e
        ...
        #collision = [@colRect.x, @colRect.y, @colRect.w, @colrect.h].intersect_rect? [e.colRect.x, e.colRect.y, e.colRect.w, e.colRect.h] 
        if collision
            #c is current entity, e is colliding entity
            @colDir.push("top")    if   ((@colRect.y + @colRect.h) > e.y) && (e.y > c.y)
            @colDir.push("bottom") if          (@colRect.y < (e.y + e.h)) && (c.y > e.y) 
            @colDir.push("right")  if    ((@colRect.x+ @colRect.w) > e.x) && (e.x > c.x)
            @colDir.push("left")   if          (@colRect.x < (e.x + e.w)) && (c.x > e.x) 
        end
