...
class Game
  ...
  def initialize args
    ...
    @player = Player.new(args: @args, id: 1, type: "player", x: 590, y: 250, w:75, h:75)
    @enemyArray  = [Enemy.new(args: @args, id: 2, type: "bat", x: 100, y: 600, movespeed: 5),
                    Enemy.new(args: @args, id: 3, type: "bat", x: 1100, y: 600, w:75, h:75),
                    Enemy.new(args: @args, id: 4, type: "goblin", x: 300, y: 600),
                    Enemy.new(args: @args, id: 5, type: "goblin", x: 400, y: 550),
                    Enemy.new(args: @args, id: 6, type: "goblin", x: 500, y: 600),
                    Enemy.new(args: @args, id: 7, type: "goblin", x: 900, y: 400),
                    Enemy.new(args: @args, id: 8, type: "slime", x: 200, y: 100, w:55, h:55),
                    Enemy.new(args: @args, id: 9, type: "slime", x: 200, y: 100, w:55, h:55),
                    Enemy.new(args: @args, id: 10, type: "slime", x: 200, y: 90, w:90, h:90),
                    Enemy.new(args: @args, id: 11, type: "slime", x: 201, y: 100, w:55, h:55)                  
                  ]
  ...
  def entityUpdate args
    ...
    @args.outputs.labels << {x: 10, y: 560, text: "x/cx/dx, x/cy/dy: [#{@player.x}/#{@player.colRect.x}/#{@player.dx}, #{@player.y}/#{@player.colRect.y}/#{@player.dy}]"}
    @args.outputs.borders << @player.colRect
    @args.outputs.borders << {x: @player.x, y: @player.y, w: @player.w, h: @player.h, r: 255, g: 0, b: 0}
  ...
  def diagnosticsLabels
    ...
    @args.outputs.labels << {x: 10, y: 680, text: "SpriteID: #{@player.spriteID}"}
    @args.outputs.labels << {x:10, y: 660, text: "Sprite: #{@player.sprite}"}
    @args.outputs.labels << {x:10, y: 640, text: "Key Pressed: #{@keypressed}"}
    @args.outputs.labels << {x:10, y: 620, text: "Animation: #{@player.spriteAnim}"}
    @args.outputs.labels << {x:10, y: 600, text: "Player.dx: #{@player.dx}"}
    @args.outputs.labels << {x:10, y: 580, text: "Player.dy: #{@player.dy}"}
    @args.outputs.labels << {x:640, y: 50, text: "Press [c] to #{@keypressed.include?("c") ? "disable" : "enable"} player collision.", alignment_enum: 1}  
  end
...
def tick args
  ...
  args.state.game.tick args.state.game.args
end