class Entity
    ...
    def initialize id:, type:, x:, y:, w:, h:, movespeed:0, spriteAnim: "idle", spriteDir: "right"
        @id = id
        @type = type
        @x =  x
        @y =  y
    ...
    def collisionPushBack
        @y -= 2 if @colDir.include?("top") && !@colDir.include?("bottom")
        @x -= 2 if @colDir.include?("right") && !@colDir.include?("left")
        @y += 2 if @colDir.include?("bottom") && !@colDir.include?("up")
        @x += 2 if @colDir.include?("left") && !@colDir.include?("right")
    end

    def collision e

        c = @colRect.intersect_rect? e.colRect
        if c && e.id != @id && (!e.is_a? Effect)
...
class Bomb < Entity
    ...
    def initialize id: , type: "bomb" , x: , y: , w: 50 , h: 50
...
class Effect < Entity
    ...
    def initialize id: , type: , x: , y: , w: 100 , h: 100
        super(id: id, type: type, x: x-25, y: y-25, w: w, h: h)
