...
def playerCollision args, p, e
  ...
  args.state.keypressed.reverse().each do |k|
    (p.colRect.y = p.y + p.dy; up = true; break) if k == "up"
    (p.colRect.x = p.x + p.dx; right = true; break) if k == "right"
    (p.colRect.y = p.y + p.dy; down = true; break)if k == "down" 
    (p.colRect.x = p.x + p.dx; left = true; break)if k == "left"
  end
