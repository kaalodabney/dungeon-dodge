...
class Entity
    attr_accessor :warp, :id, :type, :x, :y, :w, :h, :sprite, :spriteDir, :colRect, :col, :colDir, :colArray, :spriteID, :spriteAnim, :dx, :dy
...
class Enemy < Entity
    ...
    def movement p, entityArray
        ...
        (@dx = movespeed; @colDir.push("x")) if @x < p.x
        (@dx = -movespeed; @colDir.push("x")) if @x > p.x
        (@dy = movespeed; @colDir.push("y")) if @y < p.y
        (@dy = -movespeed; @colDir.push("y")) if @y > p.y



        entityArray.each do |e|
            @args.outputs.labels << {x: 10, y: 400, text: "id: #{@id}"}
            e.id == @id ? c = [false, false] : c = collision(e)
            @col[0] = true if c[0]
            @col[1] = true if c[1]
        end
    ...
    def collision e
        ...
        if @colDir.include?("x")
            xCollision = @colRect.intersect_rect? e.colRect
            @colDir.delete("x")
            
        end
        
        if @colDir.include?("y")
            yCollision = @colRect.intersect_rect? e.colRect
            @colDir.delete("y")
        end
