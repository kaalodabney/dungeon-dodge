class Entity
    attr_accessor :warp, :id, :type, :x, :y, :w, :h, :zoom, :movespeed, :sprite, :spriteDir, :colRect, :col, :colDir, :colArray, :spriteID, :spriteAnim, :dx, :dy
    def initialize id:, type:, x:, y:, w: 16, h: 16, zoom: zoom, movespeed:0, spriteAnim: "idle", spriteDir: "right"
        @id = id
        @type = type
        @x =  x
        @y =  y
        puts "xxxxxxxyyyyyyyyyyy #{@x}, #{@y}"
        @w =  w * zoom.to_i
        @h =  h * zoom.to_i
        @movespeed = movespeed
        @dx = @movespeed
        @dy = @movespeed   
        @colRect = { x: x, y: y, w: w * zoom.to_i, h: h * zoom.to_i}
        puts "cccccccccccc #{@colRect.x}"
        @col = false
...
class Player < Entity
    ...
    def initialize id:, type: "player", x:, y:, movespeed:1, zoom: zoom
...
class Bomb < Entity
    ...
    def initialize id: , type: "bomb" , x: , y: , w: 32 , h: 32, zoom:
        super(id: id, type: type, x: x, y: y, w: w, h: h, zoom: zoom)
        @damageValue ||= 25
