...
def playerCollision args, p, e
  ...
  if args.state.keypressed.include?("left")
    p.debugTog ? (p.debugVal = "colRect: #{p.colRect.dup}, p.x p.y: #{p.x.dup} #{p.y.dup}"; p.debugTog = true) : (p.debugVal = "no yet")
    p.colRect.x = p.x - p.dx
    p.debugTog2 ? (p.debugVal2 = "colRect: #{p.colRect.dup}, p.x p.y: #{p.x.dup} #{p.y.dup}"; p.debugTog = true) : (p.debugVal2 = "no yet")
    left = true
  end
