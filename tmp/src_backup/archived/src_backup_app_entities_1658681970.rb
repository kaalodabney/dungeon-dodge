...
class Enemy < Entity
    ...
    def enemyCollision e
        @colRect.x = @x + @dx*2 if @colArray.include?("right") || @colArray.include?("left")
        @colRect.y = @y + @dy*2 if @colArray.include?("up") || @colArray.include?("down")
    ...
    def movement p, entitiesArray
        ...
        entitiesArray.each do |e|
            colBuffer = enemyCollision(e) unless e.id == @id
        end
        @col = colBuffer
