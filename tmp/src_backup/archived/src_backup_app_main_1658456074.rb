...
def entityController args
  ...
  args.state.players.each do |p|
    args.outputs.labels << [10, 70, "[colRect.x, p.x]: First Left input (before collision check): #{p.debugVal3}"]
    args.outputs.labels << [10, 50, "[colRect.x, p.x]: keypressed array includes left (in playerCollision): #{p.debugVal1} (colRect.x should be 585, p.xx should be 590)"]
    args.outputs.labels << [10, 30, "left input count (in playerCollision): #{p.debugVal4}"]
