...
class Game
  ...
  def tick
    ...
    if state.tick_count.zero?
      target = $gtk.render_target(:map)
      target.width = @map.attributes.width.to_i * attributes.tilewidth.to_i
      target.height = @map.attributes.height.to_i * attributes.tileheight.to_i
