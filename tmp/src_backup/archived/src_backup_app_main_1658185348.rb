  ...
  p.colRect = {x: p.x, y: p.y, w: p.w, h: p.h}
    ...
    args.state.enemies.each do |e|
      ...
      if p.colRect.intersect_rect? e
        args.outputs.labels << [10, 500, "Collision True"]
        return [true, dir]
      else
        args.outputs.labels << [10, 500, "Collision False"]
        return [false, dir]
      end
...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    if args.inputs.right 
      args.outputs.labels << [10, 480, "#{playerCollision(args, p, "right")}"]
    ...
    p.dx = 0 if (p.col.index(0) && (p.col.index(1) == "right" || p.col.index(1) == "left"))
    p.dy = 0 if (p.col.index(0) && (p.col.index(1) == "up"    || p.col.index(1) == "down"))
