...
def actorController args
  ...
  args.state.players.each do |p|
    args.outputs.labels << [10, 400, "colRect.x: Left input (before collision check): #{p.debugVal3}"]
    args.outputs.labels << [10, 440, "colRect.x: keypressed array includes left (in playerCollision): #{p.debugVal}"]
    args.outputs.labels << [10, 380, "left input count (in playerCollision): #{p.debugVal4}"]
    args.outputs.labels << [10, 420, "colRect.x: after rectangle intersection detection (in playerCollision) - : #{p.debugVal2}"]
