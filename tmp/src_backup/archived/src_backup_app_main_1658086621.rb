...
def playerCollisionRect args, pClone, dir
    ...
    args.state.players.each do |p|
      args.state.enemies.each do |e|
        ...
        if dir == "up"
          pClone.y = p.y+p.dy
          (pClone.intersect_rect? e) ? collisionDirection = "up" : nil
        end

        if dir == "down"
          pClone.y = p.y-p.dy
          (pClone.intersect_rect? e) ? collisionDirection = "down" : nil
        end
        
        if dir == "right"
          pClone.x = p.x+p.dx
          (pClone.intersect_rect? e) ?  collisionDirection = "right" : nil
        end
        
        if dir == "left"
          pClone.x = p.x-p.dx
          (pClone.intersect_rect? e) ? collisionDirection = "left" : nil
        end
...
def playerMovement args
  ...
  args.state.players.each do |p|
    ...
    if args.inputs.up 
      ...
      playerCollisionRect(args, pClone, "up")
    ...
    if args.inputs.down 
      ...
      playerCollisionRect(args, pClone, "down")
      p.collision ? nil : p.y -= p.dy
    ...
    if args.inputs.right 
      ...
      playerCollisionRect(args, pClone, "right")
    ...
    if args.inputs.left 
      ...
      playerCollisionRect(args, pClone, "left")
      p.collision ? nil : p.x -= p.dx
