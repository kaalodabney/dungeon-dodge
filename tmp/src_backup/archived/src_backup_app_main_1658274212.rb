...
def playerCollision args, p, e
      ...
      args.state.keypressed.include?("right") ? p.colRect.x = p.x + p.dx : nil
      args.state.keypressed.include?("left") ? p.colRect.x = p.x - p.dx : nil
      args.state.keypressed.include?("up") ? p.colRect.y = p.y + p.dy : nil
      args.state.keypressed.include?("down") ? p.colRect.y = p.y - p.dy : nil
...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    args.state.enemies.each { |e| playerCollision(args, p, e) }
