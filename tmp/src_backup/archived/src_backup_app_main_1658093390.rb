...
# e.col is set to false when the player moves away from the enemey (not when player stops moving into enemy, like player collision does)
def enemyCollision(args, e)
  args.state.players.each do |p|
    if e.intersect_rect? p
      e.col = true
      e.colRect.x
    end
  end
