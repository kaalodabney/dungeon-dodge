...
class Player < Entity
    ...
    def playerMovement
        ...
        if !$game.keypressed.empty?
            $game.keypressed.each do |k|
        ...
        $game.enemyArray.each do |e| 
            if $game.keypressed.include?("c")
...
class Enemy < Entity
    ...
    def movement
        ...
        @dx = @movespeed if @x < $game.player.x
        @dx = -@movespeed if @x > $game.player.x
        @dy = @movespeed if @y < $game.player.y
        @dy = -@movespeed if @y > $game.player.y
        ...
        (colBuffer = true; $game.player.damage) if collision($game.player)
        @col = colBuffer
...
class Bomb < Entity
    ...
    def initialize id: , type: "bomb" , x: , y: , w: 50 , h: 45
        super(id: id, type: type, x: x, y: y, w: w, h: h)
        
    end

    def blowUp bomb
        $game.bombArray.delete(bomb)
        $game.entityCount -= 1
    end
end
...
class Effects < Entity
    ...
    def initialize id: , type: , x: , y: , w: 50 , h: 45
        super(id: id, type: type, x: x, y: y, w: w, h: h)
