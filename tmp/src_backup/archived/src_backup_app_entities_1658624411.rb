class Entity
    attr_accessor :id, :type, :x, :y, :w, :h, :sprite, :spriteDir, :colRect, :col, :colDir, :colArray, :spriteID, :spriteAnim, :dx, :dy
...
class Player < Entity
    ...
    def initialize args, id, x, y, w=100, h=100
...
class Enemy < Entity
    def initialize args, id, type, x, y, w=100, h=100
      super(args, type, x, y, w, h)
    end
  
    def enemyCollisionPlayer p
        collisionRL = false
        collisionUD = false
        collisionBoth = false
        collisionRL = [@colRect.x-1 + @dx, @colRect.y-1, @colRect.w+2, @colRect.h+2].intersect_rect? p.colRect
        collisionUD = [@colRect.x-1, @colRect.y-1 + @dy, @colRect.w+2, @colRect.h+2].intersect_rect? p.colRect
        collisionBoth = [@colRect.x-1 +@dx, @colRect.y-1 + @dy, @colRect.w+2, @colRect.h+2].intersect_rect? p.colRect
        @col = (collisionRL && collisionUD && collisionBoth)
    end

    def enemyCollisionEnemy e
        collision = false
        collision = [@colRect.x-1 + @dx, @colRect.y-1 + @dy, @colRect.w+2, @colRect.h+2].intersect_rect? p.colRect
        @col = collision
    end
    ...
    def enemyMovement p, enemyArray
        ...
        @x < p.x ? @dx = movespeed : @dx = -movespeed
        @y < p.y ? @dy = movespeed : @dy = -movespeed

        enemyCollisionPlayer p
        enemyArray.each { |e| enemyCollisionEnemy(e) unless e.id == @id}
