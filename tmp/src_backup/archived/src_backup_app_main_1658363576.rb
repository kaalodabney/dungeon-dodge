...
def playerCollision args, p, e
  ...
  if args.state.keypressed.include?("left")
    p.debugTog ? (p.debugVal = "colRect: #{p.colRect.dup}, p.x p.y: #{p.x.dup} #{p.y.dup}") : (p.debugVal = "no yet"; p.debugTog = true)
    p.colRect.x = p.x - p.dx
    p.debugTog2 ? (p.debugVal2 = "colRect: #{p.colRect.dup}, p.x p.y: #{p.x.dup} #{p.y.dup}") : (p.debugVal2 = "no yet"; p.debugTog2 = true)
    left = true
  end
...
def actorController args
  ...
  args.state.players.each do |p|
    args.outputs.labels << [10, 460, "debugVal: #{p.debugVal}"]
    args.outputs.labels << [10, 440, "debugVal2: #{p.debugVal2}"]
    ...
    args.state.enemies.each do |e| 
      ...
      args.outputs.labels << [10, 560, "Collision: #{playerCollision(args, p, e) }"]
      args.outputs.labels << [10, 540, "p.colCollision: #{p.col}"]
    end
    p.col = colBuffer

    args.outputs.labels << [10, 520, "p.colDir: #{p.colDir}"]
    ...
    args.state.keypressed.each do |k|
      args.outputs.labels << [10, 500, "p.colCollision3: #{p.col}"]
    ...
    args.outputs.labels << [10, 480, "p.x/p.dx, p.y/p.dy: #{p.x}/#{p.dx} #{p.y}/#{p.dy}"]
    p.y += p.dy
    p.x += p.dx
