...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    if args.inputs.up 
      args.state.enemies.each { |e| playerCollision(args, p, e, "up") ? p.col = "up"}
    ...
    if args.inputs.right 
      args.state.enemies.each { |e| playerCollision(args, p, e, "right") ? p.col = "right"}
      args.outputs.labels << [10, 500, "#{p.col}"]
    ...
    if args.inputs.down 
      args.state.enemies.each { |e| playerCollision(args, p, e, "down") ? p.col = "down"}
      args.outputs.labels << [10, 500, "#{p.col}"]
    ...
    if args.inputs.left 
      args.state.enemies.each { |e| playerCollision(args, p, e, "left") ? p.col = "left"}
      args.outputs.labels << [10, 500, "#{p.col}"]
