...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    args.outputs.labels << [10, 60, "[colRect.x, p.x]: after rectangle intersection detection (in playerCollision) - : #{p.debugVal2}"]
    ...
    upPressed ? p.dy += moveSpeed : (args.state.keypressed.delete("up") if args.state.keypressed.include?("up"))
    rightPressed ? p.dx += moveSpeed : (args.state.keypressed.delete("right") if args.state.keypressed.include?("right"))
    downPressed ? p.dy -= moveSpeed : (args.state.keypressed.delete("down") if args.state.keypressed.include?("down"))
    leftPressed ? p.dx -= moveSpeed : (args.state.keypressed.delete("left") if args.state.keypressed.include?("left"))


    p.dx = 0 if (p.col && (p.colDir & ["right", "left"]))
    p.dy = 0 if (p.col && (p.colDir & ["up", "down"]))
