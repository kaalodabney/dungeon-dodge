...
def init args
  ...
  args.state.players ||= [{
    ...
    dx: 0,
    dy: 0,                                        #3 27, 2 27, 1 23
    healthbar: {x: 615, y: 310, w: ((X * 25) + (((1/X) * 27) - ( (3/X)*(2/X) ))), h: 20, r: 225, g: 0, b: 0},
...
def enemeyCollision args, p, e
  return [e.colRect.x-1, e.colRect.y-1, e.colRect.w+2, e.colRect.h+2].intersect_rect? p.colRect
end
...
def entityController args
  ...
  args.state.players.each do |p|
    ...
    if p.healthbar.w > 5
      p.healthbar.w -= 1 if p.col
    elsif p.col
      #p.hearts = 5 if p.hearts < 1
      p.hearts -= 1 if p.hearts > 1
      p.healthbar.w = (p.hearts * 25) + 10
    end
