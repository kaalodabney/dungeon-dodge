...
def actorController args
  ...
  args.state.players.each do |p|
    ...
    args.state.keypressed.include?("up")    && p.colDir != "up" ? p.y += p.dy : nil
    args.state.keypressed.include?("down")  && p.colDir != "down"? p.y -= p.dy : nil
    args.state.keypressed.include?("right") && p.colDir != "right" ? p.x += p.dx : nil
    args.state.keypressed.include?("left")  && p.colDir != "left"? p.x -= p.dx : nil
